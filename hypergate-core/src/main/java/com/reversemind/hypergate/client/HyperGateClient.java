...
public class HyperGateClient implements IHyperGateClient, Serializable {
    ...
    private final static Logger LOG = LoggerFactory.getLogger(HyperGateClient.class);
    ...
    private FutureTask<Payload> futureTask;
    ...
    private void serverListener(Object object) {
        ...
        LOG.info("Received object from is not a Payload");
        this.futureTask.cancel(true);
        this.shutDownExecutor();
    ...
    private void setPayload(Payload inPayload) {
...
//        LOG.warn("Warmed up 1.8.0-SNAPSHOT ");
     ...
     * available.
     * <p/>
     * For purpose of HyperGateClient - just enough a single thread
     * <p/>
    ...
    private FutureTask<Payload> createFutureTask() {
        return this.futureTask = new FutureTask<Payload>(new PayloadCallable(this.payload));
    }

    /**
     *
     */
    private class PayloadCallable implements Callable<Payload> {

        private Payload callablePayload;

        PayloadCallable(Payload payload) {
            this.callablePayload = payload;
        }
